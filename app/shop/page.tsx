...
export default function Shop(){
 ...
 return <main><Header/><div className="shop-page"><span className="eyebrow">HET EMPORIUM</span><h1 className="shop-title">Het <em>Emporium</em></h1><p className="shop-intro">Ontdek speciale upgrades voor je beleving van deze server. Aankopen in het Emporium is 100% optioneel en is non-refundable</p>
  <div className="shop-tools"><label className="search-box"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Zoek in het Emporium..."/><button onClick={()=>setQuery('')} aria-label="Wis zoekopdracht"><X size={15}/></button></label><label className="sort-box"><SlidersHorizontal size={15}/><select value={sort} onChange={e=>setSort(e.target.value)}><option value="featured">Uitgelicht</option><option value="price-low">Prijs: laag → hoog</option><option value="price-high">Prijs: hoog → laag</option><option value="name">Naam A → Z</option></select></label></div>
  ...
  {loading?<div className="products">{[1,2,3,4].map(i=><div className="product-card skeleton" key={i}/>)}</div>:filtered.length?<div className="products">{filtered.map(p=><ProductCard key={p.id} product={p}/>)}</div>:<div className="empty">Geen producten gevonden. Probeer een andere zoekopdracht of categorie.</div>}
 </div></main>
}