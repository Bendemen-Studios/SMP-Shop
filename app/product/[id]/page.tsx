...
export default function ProductPage({params}:{params:Promise<{id:string}>}){
 ...
 const summary=sanitizeRichText(currentProduct.small_description)||'<p>Een exclusief onderdeel van jouw avontuur in de wereld van machtige machines.</p>';
 return <main><Header/><div className="product-page"><Link href="/shop" className="text-link"><ArrowLeft size={15}/> TERUG NAAR DE WERKPLAATS</Link><div className="product-detail">
  ...
  <motion.div className="detail-copy" initial={{opacity:0,x:25}} animate={{opacity:1,x:0}}><span className="eyebrow">{currentProduct.featured?'UITGELICHT':'PRODUCT UIT DE WERKPLAATS'}</span><h1>{currentProduct.name}</h1><div className="lead" dangerouslySetInnerHTML={{__html:summary}}/><div className="detail-price">€ {price.toFixed(2).replace('.',',')}{currentProduct.subscription?<small> / {currentProduct.duration_periodicity||'periode'}</small>:null}</div>{currentProduct.old_price&&currentProduct.old_price>price?<div className="old-price">Was € {currentProduct.old_price.toFixed(2).replace('.',',')} · {currentProduct.percent_off||Math.round((1-price/currentProduct.old_price)*100)}% voordeel</div>:null}
