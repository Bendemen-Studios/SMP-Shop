...
export default function ProductPage({params}:{params:Promise<{id:string}>}){
 const [product,setProduct]=useState<DetailedProduct|null>(null);const [selected,setSelected]=useState('');const [selectedImage,setSelectedImage]=useState('');const [qty,setQty]=useState(1);const [added,setAdded]=useState(false);const [purchaseMode,setPurchaseMode]=useState<'once'|'subscribe'>('once');const add=useCart(s=>s.add);
 ...
 if(!product)return <main><Header/><div className="empty"><div className="loading-machine">⚙</div>Product wordt uit het Emporium gehaald...</div></main>;
 ...
 function addToCart(){const server=selected?Number(selected):undefined;for(let i=0;i<qty;i++)add(currentProduct,{purchaseMode:currentProduct.subscription?purchaseMode:'once',serverSelection:server});setAdded(true);setTimeout(()=>setAdded(false),1800)}
 const description=sanitizeRichText(currentProduct.description);const summary=sanitizeRichText(currentProduct.small_description);
 return <main><Header/><div className="product-page"><Link href="/shop" className="text-link"><ArrowLeft size={15}/> TERUG NAAR HET EMPORIUM</Link><div className="product-detail">
  ...
  <motion.div className="detail-copy" initial={{opacity:0,x:25}} animate={{opacity:1,x:0}}><span className="eyebrow">{currentProduct.featured?'UITGELICHT':'PRODUCT UIT HET EMPORIUM'}</span><h1>{currentProduct.name}</h1>{summary?<div className="lead" dangerouslySetInnerHTML={{__html:summary}}/>:null}<div className="detail-price">€ {price.toFixed(2).replace('.',',')}{currentProduct.subscription?<small> / {currentProduct.duration_periodicity||'periode'}</small>:null}</div>{currentProduct.old_price&&currentProduct.old_price>price?<div className="old-price">Was € {currentProduct.old_price.toFixed(2).replace('.',',')} · {currentProduct.percent_off||Math.round((1-price/currentProduct.old_price)*100)}% voordeel</div>:null}
   ...
   {currentProduct.subscription?<div className="purchase-options"><span className="purchase-label">KIES JE AANKOOP</span><div className="purchase-choice-grid"><button type="button" className={purchaseMode==='subscribe'?'purchase-choice active':''} onClick={()=>setPurchaseMode('subscribe')}><b>Abonneren</b><small>Automatisch verlengen</small></button><button type="button" className={purchaseMode==='once'?'purchase-choice active':''} onClick={()=>setPurchaseMode('once')}><b>Los kopen</b><small>Eenmalige betaling</small></button></div></div>:null}
  ...
  <motion.section className="description-panel" initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}}><h2>Technische specificaties</h2><div className="rich-description">{description?<div dangerouslySetInnerHTML={{__html:description}}/>:<p>Productinformatie is momenteel niet beschikbaar.</p>}</div></motion.section>
 </div></main>
}