...
async function api<T>(token:string,path:string,init?:RequestInit):Promise<T>{
 const r=await fetch(`https://api.tip4serv.com/v1${path}`,{...init,headers:{Authorization:`Bearer ${token}`,Accept:'application/json','Content-Type':'application/json',...(init?.headers||{})}});
 const d=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(d?.error?.message||d?.message||d?.error||`Verzoek mislukt (${r.status})`);
 return d;
}
...
export default function Account(){
 ...
 useEffect(()=>{fetch('/api/store').then(r=>r.json()).then(d=>setStore(Number(d.id)||null)).catch(()=>{});},[]);
 useEffect(()=>{if(!store)return;const existing=document.getElementById('tip4serv-account-sdk');if(existing&&window.Tip4Serv?.OAuth){setReady(true);setLoading(false);return}const s=document.createElement('script');s.id='tip4serv-account-sdk';s.src=SDK;s.async=true;s.setAttribute('data-store-id',String(store));s.onload=()=>{setReady(true);try{if(new URLSearchParams(location.search).has('tip4serv_access_token'))window.Tip4Serv?.OAuth?.Save?.();}catch{}const t=window.Tip4Serv?.OAuth?.Token?.();if(t)setToken(t);const u=new URL(location.href);['tip4serv_access_token','error','state','code'].forEach(p=>u.searchParams.delete(p));history.replaceState({},document.title,u.pathname+u.search+u.hash);setLoading(false)};s.onerror=()=>{setLoading(false);setError('Account-login kon niet worden geladen.')};document.head.appendChild(s);},[store]);
 ...
 const connect=async()=>{setError('');if(!window.Tip4Serv?.OAuth?.Connect){setError('De accountverbinding is nog niet geladen.');return}await window.Tip4Serv.OAuth.Connect({return_url:`${location.origin}/account`});};
 ...
 if(!token)return <main><Header/><div className="account-page"><Link href="/" className="text-link"><ArrowLeft size={15}/> TERUG NAAR HOME</Link><div className="account-login"><div className="account-icon"><UserRound size={36}/></div><span className="eyebrow">MIJN ACCOUNT</span><h1 className="shop-title">Jouw <em>Emporium-account</em></h1><p>Log veilig in met je Tip4Serv-account om je aankopen te bekijken en abonnementen te beheren.</p><button className="brass-button" onClick={connect} disabled={!ready}><UserRound size={18}/> INLOGGEN</button>{error?<p className="checkout-error">{error}</p>:null}</div></div></main>;
