...
export default function Account(){
 ...
 if(!token)return <main><Header/><div className="account-page"><Link href="/" className="text-link"><ArrowLeft size={15}/> TERUG NAAR HOME</Link><div className="account-login"><div className="account-icon"><UserRound size={36}/></div><span className="eyebrow">MIJN ACCOUNT</span><h1 className="shop-title">Jouw <em>Emporium-account</em></h1><p>Log veilig in met je account om je aankopen te bekijken en abonnementen te beheren.</p><button className="brass-button" onClick={connect} disabled={!ready}><UserRound size={18}/> INLOGGEN</button>{error?<p className="checkout-error">{error}</p>:null}</div></div></main>;
